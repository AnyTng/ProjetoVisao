...
int main(void) {
    ...
    std::ofstream file("blobs_output.txt");
    if (!file.is_open()) {
        std::cerr << "Erro ao abrir o ficheiro blobs_output.txt\n";
        return 1;
    }
    ...
    while (key != 'q') {
        ...
        vc_binary_open2(image2, image3, 15, 15);

        //if (blobs != NULL) {
        //    nblobs = blobs[nblobs-1].label;
        //}
        ...
        for (int i = 0; i < nblobs; i++) {
            ...
            file << "Frame " << video.nframe << " - Blob " << id << " Label : " << blobs[i].label << " : Centro (" << centroid.x << ", " << centroid.y << ")\n";
    ...
    /*if (nblobs == 0) {
        file << "Nenhum blob detectado.\n";
    } else {
        file << "TOTAL DE BLOBS DETETADOS: " << nblobs << std::endl << std::endl;
        for (int i = 0; i < nblobs; i++) {
            file << "Blob " << i + 1 << ": Label: " << blobs[i].label << ", Area: " << blobs[i].area << ", Perimeter: " << blobs[i].perimeter << ", Centro de Massa: (" << blobs[i].xc << ", " << blobs[i].yc << ")";
            file << ", Bounding Box: [" << blobs[i].x << ", " << blobs[i].y << ", " << blobs[i].width << ", " << blobs[i].height << "]\n";
        }
    }*/
